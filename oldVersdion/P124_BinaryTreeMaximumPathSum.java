...
// 
//
// Related Topics 树 深度优先搜索 动态规划 二叉树 👍 2116 👎 0
...
public class P124_BinaryTreeMaximumPathSum {
    ...
    class Solution {
        ...
        public int dfs(TreeNode root){
            ...
            // the path sum inner the subtree whose root is "root"
            int innerMax = root.val + leftRes + rightRes;
            res =  Math.max(res, innerMax);
            // B <- A -> C
            // either A- > B
            // or A-> C
            int output = root.val + Math.max(0, Math.max(leftRes, rightRes));
