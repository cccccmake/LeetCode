        ...
        for(int row = 1; row < length; row++){
            for(int column = 0; column < triangle.get(row).size(); column++){
                if(column == triangle.get(row).size() - 1){
                    dpArray[row][column] = dpArray[row - 1][column - 1] + triangle.get(row).get(column);
                }else if (column == 0){
                    dpArray[row][column] = dpArray[row - 1][column] + triangle.get(row).get(column);
                }else{
                    dpArray[row][column] = Math.min(dpArray[row - 1][column], dpArray[row - 1][column - 1]) + triangle.get(row).get(column);
                }
            }
        }
        for(int candidate : dpArray[length - 1]){
