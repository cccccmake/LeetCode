...
class Solution {
    public int myAtoi(String s) {
        ...
        for(int i = 0; i < len; i++){
            // symbol update
            // happens iff. the hasSymbol == false
            // otherwise break directly
            if((s.charAt(i) == '-' || s.charAt(i) == '+') && !hasSymbol) {
                // even if the hasSymbol is false, if the index of the character '-' or '+'
                // larger than 0, should break directly
                // 0000+1234
                if(i > 0) break;
                hasSymbol = true;
                symbol = s.charAt(i) == '-' ? -1 : 1;
            }
            // meet numeric characters
            else if(s.charAt(i) - '0' <= 9 && s.charAt(i) - '0' >= 0){
                res = res * 10 + s.charAt(i) - '0';
                // it does be better to put the block here
                // if the block at the end, res = 0
                // WHY?
                if(res > Integer.MAX_VALUE || res < Integer.MIN_VALUE)
                    return symbol < 0 ? Integer.MIN_VALUE : Integer.MAX_VALUE;
            }
            // meet non-numeric characters, break
            else{
                break;
            }
        }
