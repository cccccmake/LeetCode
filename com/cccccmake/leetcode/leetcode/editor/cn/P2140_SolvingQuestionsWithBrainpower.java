 ...
 * @author cccccmake
 */
public class P2140_SolvingQuestionsWithBrainpower{
    public static void main(String[] args) {
        Solution solution = new P2140_SolvingQuestionsWithBrainpower().new Solution();
...
class Solution {
    public long mostPoints(int[][] questions) {
        ...
        // why long???
        long[] dpArray = new long[length + 1];
        for(int i = 0; i < length; i++){
            dpArray[i + 1] = Math.max(dpArray[i + 1], dpArray[i]);
            int[] question = questions[i];
            int j = Math.min(i + question[1] + 1, length);
            dpArray[j] = Math.max(dpArray[j], dpArray[i] + question[0]);
        }
