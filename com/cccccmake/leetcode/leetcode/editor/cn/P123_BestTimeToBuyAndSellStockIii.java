...
class Solution {
    public int maxProfit(int[] prices) {
        ...
        for(int i = 1; i < length; i++){
            ...
            dpArray[i][2] = Math.max(dpArray[i - 1][2], dpArray[i - 1][1] - prices[i]);
            dpArray[i][3] = Math.max(dpArray[i - 1][3], dpArray[i - 1][2] + prices[i]);
        }
        for(int i = 0; i < length; i++){
            for (int elem : dpArray[i]) {
                System.out.print(elem);
                System.out.print(" ");
            }
            System.out.println();
        }
        return dpArray[length - 1][3];
    }
}
